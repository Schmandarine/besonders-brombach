<?php
/**
 * besonders-brombach functions and definitions
 *
...
if ( ! function_exists( 'besonders_brombach_setup' ) ) :
	...
	function besonders_brombach_setup() {
		 ...
		 * If you're building a theme based on besonders-brombach, use a find and replace
		 * to change 'besonders-brombach' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'besonders-brombach', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'besonders-brombach' ),
			)
		);
...
function besonders_brombach_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'besonders-brombach' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'besonders-brombach' ),
...
function besonders_brombach_scripts() {
	...
	wp_enqueue_style( 'besonders-brombach-googlefonts', 'https://fonts.googleapis.com/css?family=Open+Sans:400,700,800&display=swap' );
	//wp_enqueue_style( 'besonders-brombach-fontawesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.12.1/css/all.min.css' );
	...
	wp_enqueue_script( 'jquery-scripts', get_template_directory_uri() . '/js/jquery-scripts.js', array(  ), '' , true );
